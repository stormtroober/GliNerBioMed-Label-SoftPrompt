...
# ==========================================================
def calculate_metrics(dataset, model, label_mappings, batch_size=8):
    id2name = label_mappings['id2name']
    name2desc = label_mappings['name2desc']
    desc2name = label_mappings['desc2name']
    
    # We want to report on Names (e.g. "cell type"), not IDs ("0") or Descriptions.
    # Dataset has IDs ("0").
    ...
    # Prerequisite: Model needs DESCRIPTIONS to predict
    # We will pass the list of ALL Descriptions corresponding to label_name_list
    labels_to_predict = [name2desc[name] for name in label_name_list]
    ...
    for i in tqdm(range(0, len(dataset), batch_size), desc="Evaluating"):
        ...
        for idx, item in enumerate(batch_items):
            ...
            pred_spans = set()
            for p in preds:
                # Prediction label is a DESCRIPTION. Map back to Name.
                pred_desc = p['label']
                if pred_desc not in desc2name:
                    continue # Should not happen if we passed controlled list
                
                name = desc2name[pred_desc]
                if name == 'O': continue
                
                if p['start'] in char_to_token and (p['end'] - 1) in char_to_token:
                    pred_spans.add((name, char_to_token[p['start']], char_to_token[p['end'] - 1]))
...
# ==========================================================
def main():
    parser = argparse.ArgumentParser(description="Test Bi-GLiNER with Soft Prompt Adapter")
    ...
    parser.add_argument("--label2desc", type=str, default="../dataset/label2desc.json")
    ...
    with open(args.label2desc) as f: label2desc = json.load(f)
    
    # Prepare lookup tables
    id2name = {str(v): k for k, v in label2id.items()}
    name2desc = label2desc
    desc2name = {v: k for k, v in label2desc.items()}
    ...
    # Evaluate
    macro, micro, report = calculate_metrics(test_data, model, mappings)
    
    # Save Report
    res_dir = "results"
    os.makedirs(res_dir, exist_ok=True)
    out_file = f"{res_dir}/eval_{meta.get('timestamp', 'unknown')}.md"
