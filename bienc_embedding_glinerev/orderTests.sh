...
# Presuppone che lo script venga lanciato dalla cartella bienc_embedding_glinerev
TARGET_DIR="./results"
...
if [ ! -d "$TARGET_DIR" ]; then
    ...
    echo "Assicurati di lanciare lo script dalla cartella bienc_embedding_glinerev."
    exit 1
fi
...
# Pulisce il file temporaneo (che si trova nella cartella superiore rispetto a results)
> "../$TEMP_FILE"

for file in *.md; do
    [ -e "$file" ] || continue

    # Estrae Macro F1 dalla tabella Markdown
    # Formato: | **Macro** | P | R | **F1** |
    macro_f1=$(grep "| \*\*Macro\*\*" "$file" | awk -F'|' '{print $5}' | tr -d '* \r')
    
    # Estrae Micro F1 dalla tabella Markdown
    # Formato: | **Micro** | P | R | **F1** |
    micro_f1=$(grep "| \*\*Micro\*\*" "$file" | awk -F'|' '{print $5}' | tr -d '* \r')
