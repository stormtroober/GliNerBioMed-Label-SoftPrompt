...
# LABEL2DESC_PATH is not needed since we use names, but keeping the file ref for consistency if needed.
...
def load_and_map_labels(dataset, label2id_path):
    print(f"📖 Loading label mappings from {label2id_path}")
    ...
    for label_name, idx in label2id.items():
        id2name[str(idx)] = label_name # ID -> Name (e.g. "cell type")
        
    print("🔄 Replacing numeric labels with NAMES (not descriptions)...")
    count = 0
    for item in dataset:
        for ner in item['ner']:
            # ner is [start, end, label]
            original_label = ner[2]
            if original_label in id2name:
                ner[2] = id2name[original_label]
                count += 1
                
    print(f"✅ Replaced {count} labels with names.")
    return dataset
...
if __name__ == "__main__":
    ...
    # Apply Description Mapping - MODIFIED to use only NAMES
    data = load_and_map_labels(data, LABEL2ID_PATH)
    ...
    model.train_model(
        ...
        output_dir="savings_softprompt_proj_names",
    ...
    checkpoint = {
        ...
        "training_metadata": {
            ...
            "training_mode": "names" # Indicating we used Names, not Descriptions
        },
    ...
    pt_filename = f"bi_softprompt_proj_names_adapter_{timestamp}.pt"
    ...
    if os.path.exists("savings_softprompt_proj_names"):
        print("🧹 Cleaning up temporary checkpoints in savings_softprompt_proj_names...")
        shutil.rmtree("savings_softprompt_proj_names", ignore_errors=True)
