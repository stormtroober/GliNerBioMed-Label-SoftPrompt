...
class FocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        if self.alpha is not None:
            # Crea mask per token validi (non ignore_index)
            valid_mask = targets != self.ignore_index
            alpha_t = torch.ones_like(targets, dtype=self.alpha.dtype)
            # Applica alpha solo ai token validi
...
ce_loss = FocalLoss(alpha=class_weights if FOCAL_ALPHA else None, gamma=FOCAL_GAMMA, ignore_index=-100)
...
save_path = f"savings/soft_embeddings_bs{BATCH_SIZE}_ep{EPOCHS}_lr{LEARNING_RATE}_wd{WEIGHT_DECAY}_temp{TEMPERATURE}_gc{GRAD_CLIP}_warmup{WARMUP_STEPS}_patience{EARLY_STOPPING_PATIENCE}_focal_alpha{FOCAL_ALPHA}_gamma{FOCAL_GAMMA}.pt"
