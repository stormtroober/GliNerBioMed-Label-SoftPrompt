...
def compute_class_weights(data_path, label2id):
    ...
    for record in data:
        for label in record["labels"]:
            if label != -100: counts[label] += 1; total += 1
    return total / (len(label2id) * counts.clamp(min=1))
...
# Creazione lista parametri dinamica
...
print(f"\n🚀 Inizio Training | Projection Trainable: {TRAIN_PROJECTION}")
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch in pbar:
        ...
        # 3. Label Encoding (Frozen Backbone)
        ...
        # 4. Projection (Trainable o Frozen a seconda del config)
    ...
    if avg_loss < best_loss:
        ...
        # Salviamo sempre il prompt encoder
        best_model_state = {
            'prompt_encoder': prompt_encoder.state_dict(),
            'config': {'train_projection': TRAIN_PROJECTION} # Salviamo info config
        }
        
        # Salviamo la projection SOLO se è stata allenata
        if TRAIN_PROJECTION:
            best_model_state['projection'] = proj.state_dict()
        
        # Spostiamo su CPU per il salvataggio
...
# Salvataggio finale con nome file parlante
if best_model_state is not None:
    os.makedirs("savings", exist_ok=True)
    
    # Aggiunta tag PROJ nel filename
    proj_tag = "PROJ-TRUE" if TRAIN_PROJECTION else "PROJ-FALSE"
    
    filename = f"mlp_prompt_{proj_tag}_lr{LEARNING_RATE}_ep{EPOCHS}_temp{TEMPERATURE}_best.pt"
    save_path = os.path.join("savings", filename)
    
    torch.save(best_model_state, save_path)
    print(f"💾 Modello salvato in {save_path}")
    if not TRAIN_PROJECTION:
        print("ℹ️  (Nota: È stato salvato solo il prompt_encoder perché la projection era frozen)")
else:
